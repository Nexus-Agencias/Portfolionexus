...
export default function FluidCursor({
  ...
  useEffect(() => {
    const mq = window.matchMedia(
      '(pointer: fine) and (hover: hover) and (min-width: 768px) and (prefers-reduced-motion: no-preference)',
    )
  ...
  useEffect(() => {
    ...
    if (!api) return
    api.config.SIM_RESOLUTION = simResolution
    api.config.DYE_RESOLUTION = dyeResolution
    api.initFramebuffers()
  }, [simResolution, dyeResolution])

  useEffect(() => {
    const api = apiRef.current
    if (!api) return
    api.config.SHADING = shading
    api.updateKeywords()
  }, [shading])
  ...
  return (
    <div
      className={`pointer-events-none fixed top-0 left-0 z-50 size-full${className ? ` ${className}` : ''}`}
    >
      <canvas id="fluid" ref={canvasRef} className="pointer-events-none block h-screen w-screen" />
    </div>
  )
}
